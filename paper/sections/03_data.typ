...
The register stores each inspection as an integer grade from 1 to 4 and shows the four most recent grades per establishment alongside the address, postcode, and a Danish industry classifier (branche). The 2022 reform retired grade 3 from routine use, although a small number of legacy grade-3 rows still appear in the snapshot, but they are dropped in the analysis. In the post-reform data, grade 1 maps to the happy pictogram, grade 2 to neutral, and grade 4 to sad @foedevarestyrelsen_findsmiley.
...
Building the dataset meant querying publicly visible Google reviews one place at a time, since no Danish dataset links review text to inspection outcomes. The reviews are already public, and we collected only the fields the research question needs. Google returns a reviewer name alongside each review body, but we did not store it. The retained fields are star rating, review text, and timestamp, in line with the General Data Protection Regulation @gdpr2016regulation. We keep the scraped corpus locally and publish only our code and aggregate outputs, and none of the raw scraped review data is uploaded to the public repository.
