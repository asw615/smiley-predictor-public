...
We ran the LLM only on reviews with 1, 2, or 3 stars and treated 4- and 5-star reviews as carrying no hygiene flags. We did this because running Gemma over the full review corpus was too computationally expensive, and the high-star test (a 0.9% flag rate on the 1,000-review sample, see @sec-annotation) suggested the cost of skipping them was small. The test only examined whether high-rated reviews carry a flag at all, and not how informative those flags would be.
...
The Danish inspection cadence is publicly documented and risk-tiered @foedevarestyrelsen_bekendtgoerelse. A restaurant that knows roughly when its next visit will fall can clean up shortly before. Re-ordering the scheduled queue is of limited value, because operators who know roughly when their next visit will fall can clean up shortly before. A review-based predictor is more useful for flagging restaurants whose reviews suggest current hygiene problems the next scheduled visit will not catch in time.
