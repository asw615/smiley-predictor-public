...
Logistic regression matches or beats XGBoost on every metric. On not-happy PR-AUC the summary-feature LR scores 0.182 against 0.174 for XGBoost, with overlapping bootstrap intervals. The LR wins on point estimate but the intervals overlap, so we do not claim a separation. The clearest XGBoost shortfall is on the sad grade, where its PR-AUC of 0.021 barely clears the 0.020 base rate while LR reaches 0.027. That, together with the easier reading of a linear model's coefficients, is why we treat the summary-feature logistic regression as the best of the four models.

This lift comes from a sample where bad grades appear more often than in the original Smiley register. Our panel contains 14.9% non-happy smileys, while the comparable population slice in the Smiley register with restaurants in the same category after 2022 pooled across the three retained inspection slots, sits at 9.3%. Precision depends on how common bad grades are in the data, so performance would likely be somewhat lower on the underlying population. Because the difference between the two prevalence rates is modest, we expect that reduction to be limited.
...
Augmenting Google Maps with other review platforms (Trustpilot, Yelp, or TripAdvisor) could in principle reduce the empty-window rate. We did not pursue this option for two reasons. Neither of the platforms have the coverage Google Maps does for Danish restaurants, and cross-platform aggregation would risk duplicating evidence when a single diner posts about the same visit on multiple sites. De-duplication across platforms requires either reviewer-identity matching, which Google Maps reviews do not expose.
...
We ran the LLM only on reviews with 1, 2, or 3 stars and treated 4- and 5-star reviews as carrying no hygiene flags. We did this because running Gemma over the full review corpus was too computationally expensive, and the high-star test (0.9% false-positive rate, see @sec-annotation) suggested the cost of skipping them was small. The test only bounds examined how many high-star reviews carry a true flag, and not how informative those flags would be, if we had them.
