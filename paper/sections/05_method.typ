...
Each between-inspection review window is summarised by three features (@tab-summary-features). 
...
#figure(
  table(
    ...
    [mean star rating], [arithmetic mean of in-window star ratings],
...
A class-frequency baseline predicts the training-fold class proportions for every test row. This establishes the floor any feature set has to beat.

The main comparison is a 2×2 design over feature sets and classifiers. The two feature sets are the three summary features alone, and the summary features combined with the six hygiene flags. The classifiers are multinomial logistic regression (LR) and XGBoost @chen2016xgboost. Each of the four combinations is fit per fold and pooled out-of-fold for evaluation.
...
At a 14.9% non-happy rate, a model can achieve high ROC-AUC while placing few non-happy inspections near the top of the predicted ranking, so we use PR-AUC as the primary metric @saito2015precision. We report it per class in a one-vs-rest framing, and as a non-happy score defined as one minus the predicted happy probability. We also report precision at the top decile of that ranking.
...
For each hygiene category, we fit a separate pairwise logistic regression to estimate whether a positive flag for that category in a window raises the odds of a neutral or sad inspection over a happy one. We control for log(1 + scored review count), because a flag is more likely to be positive when more reviews were scored. This gives twelve odds ratios, one for each of the six categories times the two non-happy classes. Confidence intervals come from the same restaurant-clustered bootstrap used for the cross-validated metrics. We report the twelve odds ratios as exploratory rather than confirmatory. The six hygiene flags are correlated across categories, so a standard Bonferroni correction would be over-conservative, and we instead report each estimate with its bootstrap confidence interval and interpret the table against the expected false-positive count under the null.
